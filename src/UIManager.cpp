...
void UIManager::updateCurrentWeather(const char* icon, float temperature, float feelsLike) {
    if (currentWeatherIcon && currentTempLabel && feelsLikeLabel) {
        // Update weather icon (using Unicode emoji for now)
        // In a more complete implementation, we would map OpenWeatherMap icon codes to appropriate icons
        if (icon) {
            // Simple mapping of OpenWeatherMap icon codes to Unicode emoji
            // This is a simple implementation - in a production app you'd want to use actual weather icons
            if (strstr(icon, "01")) {
                // Clear sky
                lv_label_set_text(currentWeatherIcon, "☀️");
            } else if (strstr(icon, "02")) {
                // Few clouds
                lv_label_set_text(currentWeatherIcon, "🌤️");
            } else if (strstr(icon, "03") || strstr(icon, "04")) {
        ...
        lv_label_set_text(currentTempLabel, tempBuf);
        
        // Update feels like temperature
        char feelsBuf[32];
        snprintf(feelsBuf, sizeof(feelsBuf), "Feels: %.1f°C", feelsLike);
        lv_label_set_text(feelsLikeLabel, feelsBuf);
        
        // Refresh the UI elements
        lv_obj_invalidate(currentWeatherIcon);
        lv_obj_invalidate(currentTempLabel);
        lv_obj_invalidate(feelsLikeLabel);
    }
}

void UIManager::updateMorningForecast(const char* icon, float temperature, int rainProb) {
    if (morningForecastIcon && morningTempLabel && morningRainLabel) {
        // Update forecast icon (using Unicode emoji for now)
        if (icon) {
            // Simple mapping of OpenWeatherMap icon codes to Unicode emoji
            if (strstr(icon, "01")) {
                // Clear sky
                lv_label_set_text(morningForecastIcon, "☀️");
            } else if (strstr(icon, "02")) {
                // Few clouds
                lv_label_set_text(morningForecastIcon, "🌤️");
            } else if (strstr(icon, "03") || strstr(icon, "04")) {
        ...
        char tempBuf[16];
        snprintf(tempBuf, sizeof(tempBuf), "%.1f°C", temperature);
        lv_label_set_text(morningTempLabel, tempBuf);
        
        // Update rain probability
        char rainBuf[32];
        snprintf(rainBuf, sizeof(rainBuf), "Rain: %d%%", rainProb);
        lv_label_set_text(morningRainLabel, rainBuf);
        
        // Refresh the UI elements
        lv_obj_invalidate(morningForecastIcon);
        lv_obj_invalidate(morningTempLabel);
        lv_obj_invalidate(morningRainLabel);
    }
}

void UIManager::updateAfternoonForecast(const char* icon, float temperature, int rainProb) {
    if (afternoonForecastIcon && afternoonTempLabel && afternoonRainLabel) {
        // Update forecast icon (using Unicode emoji for now)
        if (icon) {
            // Simple mapping of OpenWeatherMap icon codes to Unicode emoji
            if (strstr(icon, "01")) {
                // Clear sky
                lv_label_set_text(afternoonForecastIcon, "☀️");
            } else if (strstr(icon, "02")) {
                // Few clouds
                lv_label_set_text(afternoonForecastIcon, "🌤️");
            } else if (strstr(icon, "03") || strstr(icon, "04")) {
        ...
        char tempBuf[16];
        snprintf(tempBuf, sizeof(tempBuf), "%.1f°C", temperature);
        lv_label_set_text(afternoonTempLabel, tempBuf);
        
        // Update rain probability
        char rainBuf[32];
        snprintf(rainBuf, sizeof(rainBuf), "Rain: %d%%", rainProb);
        lv_label_set_text(afternoonRainLabel, rainBuf);
        
        // Refresh the UI elements
        lv_obj_invalidate(afternoonForecastIcon);
        lv_obj_invalidate(afternoonTempLabel);
        lv_obj_invalidate(afternoonRainLabel);
    }
}
...
void UIManager::createHomeScreen() {
    ...
    lv_obj_set_size(weatherPanel, 200, 320);
    ...
    lv_obj_set_style_bg_opa(weatherPanel, LV_OPA_50, LV_PART_MAIN);
    lv_obj_set_style_border_width(weatherPanel, 2, LV_PART_MAIN);
    lv_obj_set_style_border_color(weatherPanel, lv_color_hex(0x0000FF), LV_PART_MAIN);
    lv_obj_set_style_radius(weatherPanel, 10, LV_PART_MAIN);
    lv_obj_set_style_pad_all(weatherPanel, 10, LV_PART_MAIN);
    
    // Style for weather box titles
    static lv_style_t weather_title_style;
    lv_style_init(&weather_title_style);
    lv_style_set_text_font(&weather_title_style, &lv_font_montserrat_14);
    lv_style_set_text_color(&weather_title_style, lv_color_white());
    
    // Style for weather values
    static lv_style_t weather_value_style;
    lv_style_init(&weather_value_style);
    lv_style_set_text_font(&weather_value_style, &lv_font_montserrat_20);
    lv_style_set_text_color(&weather_value_style, lv_color_white());
    
    // Current weather box
    currentWeatherPanel = lv_obj_create(weatherPanel);
    lv_obj_set_size(currentWeatherPanel, 180, 100);
    lv_obj_align(currentWeatherPanel, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_set_style_bg_color(currentWeatherPanel, lv_color_hex(0x333333), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(currentWeatherPanel, LV_OPA_70, LV_PART_MAIN);
    lv_obj_set_style_border_width(currentWeatherPanel, 1, LV_PART_MAIN);
    lv_obj_set_style_border_color(currentWeatherPanel, lv_color_hex(0x444444), LV_PART_MAIN);
    lv_obj_set_style_radius(currentWeatherPanel, 8, LV_PART_MAIN);
    
    // Title for current weather
    lv_obj_t* currentWeatherTitle = lv_label_create(currentWeatherPanel);
    lv_obj_add_style(currentWeatherTitle, &weather_title_style, 0);
    lv_label_set_text(currentWeatherTitle, "CURRENT WEATHER");
    lv_obj_align(currentWeatherTitle, LV_ALIGN_TOP_MID, 0, 5);
    
    // Weather icon placeholder
    currentWeatherIcon = lv_label_create(currentWeatherPanel);
    lv_obj_set_style_text_font(currentWeatherIcon, &lv_font_montserrat_32, 0);
    lv_label_set_text(currentWeatherIcon, "☀️"); // Default placeholder icon
    lv_obj_align(currentWeatherIcon, LV_ALIGN_LEFT_MID, 15, 10);
    
    // Current temperature
    currentTempLabel = lv_label_create(currentWeatherPanel);
    lv_obj_add_style(currentTempLabel, &weather_value_style, 0);
    lv_label_set_text(currentTempLabel, "--°C");
    lv_obj_align(currentTempLabel, LV_ALIGN_RIGHT_MID, -15, -10);
    
    // Feels like temperature
    feelsLikeLabel = lv_label_create(currentWeatherPanel);
    lv_obj_set_style_text_font(feelsLikeLabel, &lv_font_montserrat_14, 0);
    lv_label_set_text(feelsLikeLabel, "Feels: --°C");
    lv_obj_align(feelsLikeLabel, LV_ALIGN_BOTTOM_RIGHT, -15, -5);
    
    // Morning forecast box
    morningForecastPanel = lv_obj_create(weatherPanel);
    lv_obj_set_size(morningForecastPanel, 180, 90);
    lv_obj_align(morningForecastPanel, LV_ALIGN_TOP_MID, 0, 110);
    lv_obj_set_style_bg_color(morningForecastPanel, lv_color_hex(0x333333), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(morningForecastPanel, LV_OPA_70, LV_PART_MAIN);
    lv_obj_set_style_border_width(morningForecastPanel, 1, LV_PART_MAIN);
    lv_obj_set_style_border_color(morningForecastPanel, lv_color_hex(0x444444), LV_PART_MAIN);
    lv_obj_set_style_radius(morningForecastPanel, 8, LV_PART_MAIN);
    
    // Title for morning forecast
    lv_obj_t* morningForecastTitle = lv_label_create(morningForecastPanel);
    lv_obj_add_style(morningForecastTitle, &weather_title_style, 0);
    lv_label_set_text(morningForecastTitle, "MORNING FORECAST");
    lv_obj_align(morningForecastTitle, LV_ALIGN_TOP_MID, 0, 5);
    
    // Morning forecast icon
    morningForecastIcon = lv_label_create(morningForecastPanel);
    lv_obj_set_style_text_font(morningForecastIcon, &lv_font_montserrat_32, 0);
    lv_label_set_text(morningForecastIcon, "☁️"); // Default placeholder icon
    lv_obj_align(morningForecastIcon, LV_ALIGN_LEFT_MID, 15, 10);
    
    // Morning temperature
    morningTempLabel = lv_label_create(morningForecastPanel);
    lv_obj_add_style(morningTempLabel, &weather_value_style, 0);
    lv_label_set_text(morningTempLabel, "--°C");
    lv_obj_align(morningTempLabel, LV_ALIGN_RIGHT_MID, -15, -10);
    
    // Morning rain probability
    morningRainLabel = lv_label_create(morningForecastPanel);
    lv_obj_set_style_text_font(morningRainLabel, &lv_font_montserrat_14, 0);
    lv_label_set_text(morningRainLabel, "Rain: --%");
    lv_obj_align(morningRainLabel, LV_ALIGN_BOTTOM_RIGHT, -15, -5);
    
    // Afternoon forecast box
    afternoonForecastPanel = lv_obj_create(weatherPanel);
    lv_obj_set_size(afternoonForecastPanel, 180, 90);
    lv_obj_align(afternoonForecastPanel, LV_ALIGN_TOP_MID, 0, 210);
    lv_obj_set_style_bg_color(afternoonForecastPanel, lv_color_hex(0x333333), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(afternoonForecastPanel, LV_OPA_70, LV_PART_MAIN);
    lv_obj_set_style_border_width(afternoonForecastPanel, 1, LV_PART_MAIN);
    lv_obj_set_style_border_color(afternoonForecastPanel, lv_color_hex(0x444444), LV_PART_MAIN);
    lv_obj_set_style_radius(afternoonForecastPanel, 8, LV_PART_MAIN);
    
    // Title for afternoon forecast
    lv_obj_t* afternoonForecastTitle = lv_label_create(afternoonForecastPanel);
    lv_obj_add_style(afternoonForecastTitle, &weather_title_style, 0);
    lv_label_set_text(afternoonForecastTitle, "AFTERNOON FORECAST");
    lv_obj_align(afternoonForecastTitle, LV_ALIGN_TOP_MID, 0, 5);
    
    // Afternoon forecast icon
    afternoonForecastIcon = lv_label_create(afternoonForecastPanel);
    lv_obj_set_style_text_font(afternoonForecastIcon, &lv_font_montserrat_32, 0);
    lv_label_set_text(afternoonForecastIcon, "🌤️"); // Default placeholder icon
    lv_obj_align(afternoonForecastIcon, LV_ALIGN_LEFT_MID, 15, 10);
    
    // Afternoon temperature
    afternoonTempLabel = lv_label_create(afternoonForecastPanel);
    lv_obj_add_style(afternoonTempLabel, &weather_value_style, 0);
    lv_label_set_text(afternoonTempLabel, "--°C");
    lv_obj_align(afternoonTempLabel, LV_ALIGN_RIGHT_MID, -15, -10);
    
    // Afternoon rain probability
    afternoonRainLabel = lv_label_create(afternoonForecastPanel);
    lv_obj_set_style_text_font(afternoonRainLabel, &lv_font_montserrat_14, 0);
    lv_label_set_text(afternoonRainLabel, "Rain: --%");
    lv_obj_align(afternoonRainLabel, LV_ALIGN_BOTTOM_RIGHT, -15, -5);
