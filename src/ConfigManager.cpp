...
bool ConfigManager::begin() {
    ...
    if (!SD_MMC.exists(CONFIG_FILE)) {
        if (!SPIFFS.exists(CONFIG_TEMPLATE)) {
            Serial.println("No config template found, creating default config");
            setDefaultConfig();
            return saveConfig();
        }
        
        // Copy template to SD card
        File src = SPIFFS.open(CONFIG_TEMPLATE);
        File dst = SD_MMC.open(CONFIG_FILE, FILE_WRITE);
        
        if (!src || !dst) {
            Serial.println("Failed to copy config template");
            return false;
        }
...
bool ConfigManager::saveConfig() {
    ...
    File tempFile = SD_MMC.open("/temp_config.json", FILE_WRITE);
    if (!tempFile) {
    ...
    if (serializeJson(doc, tempFile) == 0) {
        ...
        tempFile.close();
        SD_MMC.remove("/temp_config.json");
        return false;
    }
